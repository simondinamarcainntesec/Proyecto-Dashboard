...
const PALETTE=["#60A5FA","#8B5CF6","#10B981","#F59E0B","#06B6D4","#F97316","#22C55E","#EAB308","#2563EB","#DC2626","#0EA5E9","#A855F7"];
...
export function renderCountries(){
  ...
  const canvas = document.getElementById("chartCountries"); if (!canvas) return;
  ...
  const conf = {
    type:"doughnut",
    data:{ labels, datasets:[{ data:values, backgroundColor:backgroundColors, borderColor:"#e5e7eb", hoverBorderColor:"#e5e7eb", borderWidth:2, hoverBorderWidth:2, hoverOffset:8 }] },
    options:{ cutout:"62%", animation:{ duration:600, easing:"easeOutQuart" },
      plugins:{ title:{ display:true, text:titleText, color:TXT, font:{ size:16, weight:"700" }, padding:{ top:4, bottom:8 } }, legend:{ display:false }, tooltip:{ enabled:true } }
    }
  };
  ...
  if (!chart){
    ...
    ctx.canvas.onclick=(evt)=>{
      ...
      if(!els.length) return; const idx=els[0].index; actions.toggleCountry?.(labels[idx]);
    };
  ...
  if(sameLabels && sameValues){
    ...
    chart.options.plugins.title.text=titleText; chart.$static={labels:labels.slice(), values:values.slice()};
    chart.update();
  }
}
