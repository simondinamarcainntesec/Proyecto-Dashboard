...
import { selectTopDevicesPayload } from "/static/js/soar/selectors.js";

let chart;
export function renderTopDevices(){
  const payload = selectTopDevicesPayload(10, true); // ignora su propio filtro al calcularse
  const el = document.getElementById("chartDevices"); if(!el) return;
  ...
  if(!chart){
    ...
    ctx.canvas.onclick=(evt)=>{
      ...
      actions.toggleDevice?.(labels[idx]);
    };
  } else {
