...
import { selectTopProtoPayload } from "/static/js/soar/selectors.js";

let chart;
export function renderTopProto(){
  const payload = selectTopProtoPayload(10, true); // ignora su propio filtro
  const el = document.getElementById("chartProto"); if(!el) return;
  ...
  if(!chart){
    ...
    ctx.canvas.onclick=(evt)=>{
      ...
      actions.toggleProto?.(labels[idx]);
    };
  } else {
