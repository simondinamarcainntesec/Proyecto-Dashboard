...
export function renderSeverity(){
  ...
  const el = document.getElementById("chartSeverity"); if (!el) return;
  ...
  const conf = {
    ...
    options: {
      ...
      plugins:{ title:{ display:true, text:titleText, color:titleColor, font:{ size:16, weight:"700" }, padding:{ top:4, bottom:8 } }, legend:{ display:false }, tooltip:{ enabled:true } }
    },
  };
  ...
  if (sameLabels && sameValues){
    ...
    chart.options.plugins.title.text = titleText; chart.options.plugins.title.color = titleColor;
    chart.update("none");
  } else {
