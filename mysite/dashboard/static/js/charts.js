(function () {
  ...
  // Buscar clave case-insensitive y devolver la real
  ...
  // Importante para el donut al filtrar por acción:
  ...
  // Índices desde actionBySevRaw:
  //   - sevToAct: severidad -> { accion: count }
  //   - actToSev: accion    -> { severidad: count }
  const sevToAct = {};
  const actToSev = {};
  for (const [outerKey, innerMap] of Object.entries(actionBySevRaw || {})) {
    const sevKey = String(outerKey).trim(); // usamos la severidad tal cual para labels
    for (const [actRaw, val] of Object.entries(innerMap || {})) {
      const actKey = norm(actRaw);
      const n = Number(val || 0);
      // sev -> act
      if (!sevToAct[sevKey]) sevToAct[sevKey] = {};
      sevToAct[sevKey][actKey] = (sevToAct[sevKey][actKey] || 0) + n;
      // act -> sev
      if (!actToSev[actKey]) actToSev[actKey] = {};
      actToSev[actKey][sevKey] = (actToSev[actKey][sevKey] || 0) + n;
    }
  }
  ...
  function canonicalDeviceKey(input) {
    ...
    for (const k of deviceKeys) {
      if (String(k).trim().toLowerCase() === needle) return k;
    }
  ...
  function severityCountsForMsgSeverity(msg) {
    const m =
      severityByMsgSeverityRaw && severityByMsgSeverityRaw[msg] ? severityByMsgSeverityRaw[msg] : {};
    const out = {};
    Object.keys(m).forEach((sev) => (out[String(sev).trim()] = Number(m[sev] || 0)));
    return out;
  }
  ...
  function calcDonut() {
    ...
    const colors = labels.map((k) =>
      !severityFilter || String(k).trim().toLowerCase() === String(severityFilter).trim().toLowerCase()
        ? colorFor(k)
  ...
  function trendDataForCurrentFilter() {
    ...
    const datasets = Object.keys(severityTrendsMap).map((sev) => {
      ...
      const hidden =
        severityFilter && sevKey.toLowerCase() !== String(severityFilter).trim().toLowerCase();
      const base = colorFor(sevKey);
      return {
        label: sevKey,
        data:
          (severityTrendsMap[sev] && severityTrendsMap[sev].data) ||
  ...
  function deviceRowsForCurrentFilter() {
    ...
    // filtrar por ACCIÓN usando deviceByAction
    ...
    return Object.entries(deviceCountsAll)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 10);
  }
  ...
  function actionDataForCurrentFilter() {
    ...
    if (severityFilter) {
      const realSev = findKeyCI(sevToAct, severityFilter);
      if (realSev && sevToAct[realSev]) {
        const m = sevToAct[realSev];
        const keys = Object.keys(m);
        const labels = keys.map((k) => prettyActionLabel(k));
        const data = keys.map((k) => Number(m[k] || 0));
        return { labels, data, keys };
  ...
  function msgSeverityDataForCurrentFilter() {
    ...
    if (actionFilter && actionByMsgSeverityRaw) {
      ...
      Object.entries(actionByMsgSeverityRaw).forEach(([msg, perAct]) => {
        const per = perAct || {};
        const real = findKeyCI(per, actionFilter) ?? actionFilter; // busca la clave real (case-insensitive)
  ...
  // ===== Render =====
  let trendChart, actionBarChart, hourlyChart, msgSeverityBarChart;
      ...
      plugins: {
        legend: {
          ...
          onClick: (e, item) => {
            ...
            hourFilter = "";
            msgSeverityFilter = "";
            severityFilter = severityFilter.toLowerCase() === sev.toLowerCase() ? "" : sev;
            updateAll();
          },
        },
  ...
  const titleText =
  severityFilter
    ? `Severidad: ${safeLabel(severityFilter)}`
    : actionFilter
      ? `Acción: ${prettyActionLabel(actionFilter)}`
      : deviceFilter
        ? `Dispositivo: ${canonicalDeviceKey(deviceFilter)}`
        : msgSeverityFilter
          ? `Msg severity: ${msgSeverityFilter}`
          : "Severidad";

    const titleColor = severityFilter ? colorFor(severityFilter) : TXT;
    ...
    const opts = {
      ...
      plugins: {
        ...
        legend: {
          display: false, // 👈 oculta por completo la leyenda
        },
        tooltip: {
          enabled: true,
          callbacks: {
    ...
    if (!window.donutChart) {
      ...
      ctx.canvas.onclick = (evt) => {
        const el = window.donutChart.getElementsAtEventForMode(
          evt,
          "nearest",
          { intersect: true },
          true
        ...
        msgSeverityFilter = "";
        severityFilter =
          String(severityFilter).trim().toLowerCase() === String(sev).trim().toLowerCase()
            ? ""
            : sev;
        updateAll();
      };
  ...
  function renderActionBar() {
    ...
    if (!actionBarChart) {
      ...
      ctx.canvas.onclick = (evt) => {
        const el = actionBarChart.getElementsAtEventForMode(
          evt,
          "nearest",
          { intersect: true },
          true
  ...
  function renderMsgSeverityBar() {
    ...
    const backgroundColors = labels.map((label) => {
      if (msgSeverityFilter && label !== msgSeverityFilter) return "rgba(255,255,255,0.18)";
    ...
    if (!msgSeverityBarChart) {
      ...
      ctx.canvas.onclick = (evt) => {
        const elp = msgSeverityBarChart.getElementsAtEventForMode(
          evt,
          "nearest",
          { intersect: true },
          true
  ...
  function renderHourly() {
    ...
    if (!hourlyChart) {
      ...
      ctx.canvas.onclick = (evt) => {
        const points = hourlyChart.getElementsAtEventForMode(
          evt,
          "nearest",
          { intersect: true },
          true
