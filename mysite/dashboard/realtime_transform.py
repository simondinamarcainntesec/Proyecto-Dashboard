...
def build_realtime_context(alarms: list[dict], value_for_column, tzname="America/Santiago") -> dict:
    ...
    sev_by_hour               = defaultdict(Counter)
    dev_by_hour               = defaultdict(Counter)
    act_by_hour               = defaultdict(Counter)
    level_by_hour             = defaultdict(Counter)
    subtype_by_hour           = defaultdict(Counter)
    msgsev_by_hour            = defaultdict(Counter)

    # series por hora
    sev_series_by_hour        = defaultdict(Counter)
    dev_series_by_hour        = defaultdict(Counter)
    act_series_by_hour        = defaultdict(Counter)
    msgsev_series_by_hour     = defaultdict(Counter)

    # cruces
    device_by_action          = defaultdict(Counter)
    action_by_device          = defaultdict(Counter)

    device_by_severity        = defaultdict(Counter)
    device_by_level           = defaultdict(Counter)
    device_by_subtype         = defaultdict(Counter)
    device_by_logdesc         = defaultdict(Counter)

    action_by_level           = defaultdict(Counter)
    severity_by_level         = defaultdict(Counter)
    level_by_msgseverity      = defaultdict(Counter)
    msgseverity_by_level      = defaultdict(Counter)
    level_by_subtype          = defaultdict(Counter)

    severity_by_subtype       = defaultdict(Counter)
    action_by_subtype         = defaultdict(Counter)
    msgseverity_by_subtype    = defaultdict(Counter)
    subtype_by_msgseverity    = defaultdict(Counter)

    action_by_msgseverity     = defaultdict(Counter)
    severity_by_msgseverity   = defaultdict(Counter)
    device_by_msgseverity     = defaultdict(Counter)

    action_by_logdesc         = defaultdict(Counter)
    severity_by_logdesc       = defaultdict(Counter)

    severity_by_action        = defaultdict(Counter)
    action_by_severity        = defaultdict(Counter)
    ...
    _device_by_sev_full = _transpose(device_by_severity)
    ...
    severity_counts_by_msg_severity = _transpose(severity_by_msgseverity)
    msg_severity_by_subtype_out     = {st: dict(c) for st, c in subtype_by_msgseverity.items()}
    subtype_by_msg_severity_out     = {msg: dict(c) for msg, c in msgseverity_by_subtype.items()}

    level_to_device   = _transpose(device_by_level)
    subtype_to_device = _transpose(device_by_subtype)
    ...
    out = {
        ...
        "subtype_counts_by_level": subtype_counts_by_level,
        ...
        "device_counts_by_subtype": subtype_to_device,
    ...
    out.update({
        ...
        "levelBySubtypeRaw":              out.get("level_counts_by_subtype", {}),
